...
uint16_t inputRegs[1] = {0};   // Input Register 0 = humedad * 10
...
// ---------------------Read Input Register Handler FC04 ----------------------
...
void setup()
{
  ...
  Serial.begin(115200); // CAMBIADO: antes 9600 → causaba puntos en el monitor
  while (!Serial)
  {
  }
...
void loop()
{
  ...
  static bool lastValue = false; // Aplica cambios de coil. TODO: global machen? 
  if (coilTrigger)
  {
...
delay(1000);
  ...
  //Serial.printf("Humedad: %.1f %%\n", humidityValue); // Sensor en el Monitor
