...
void setup()
{
  ...
  //   pinMode(GPIO_LED, OUTPUT); // LED beim supermini
  // #else
  ...
  // #ifdef BOARD_SUPERMINI
  //   digitalWrite(GPIO_LED, HIGH); // LED beim supermini
  // #else
  ...
  // bool prog_state = digitalRead(GPIO_PROG);

  // bool state = digitalRead(GPIO_BUTTON); //obsolet
...
void loop()
{
  ...
  if (coilTrigger)
  {
    ...
    { // enthält die empfangene Nachricht einen neuen Wert für die Coil?
      ...
      // #ifdef BOARD_SUPERMINI
      //       digitalWrite(GPIO_LED, myCoils[0] ? HIGH : LOW); // LED beim supermini
      // #else
